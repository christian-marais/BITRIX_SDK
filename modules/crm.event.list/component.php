...
class NSContactEventActivity {
    private $B24 = null;
    private $NSContactEventActivityCollection = null;
    ...
    public function __construct() {
        $this->NSContactEventActivityCollection = new \stdClass();
    ...
    public function getContactEventActivities() {
        try {
            ...
            $this->NSContactEventActivityCollection->activities = $result;
            $this->NSContactEventActivityCollection->pagination = [
                'total' => $totalCount,
            ...
            $this->NSContactEventActivityCollection->activities = [];
            $this->NSContactEventActivityCollection->pagination = [
                'total' => 0,
                'itemsPerPage' => $this->itemsPerPage,
                'currentPage' => $this->currentPage,
    ...
    public function getResponsible($userId){
        $this->NSContactEventActivityCollection->responsible=null;
        try{
            if (!is_numeric($userId) || !$this->hasScope('user') ||!$this->NSContactEventActivityCollection->responsible = $this->B24?->core?->call('user.get', [
    ...
    public function renderEventActivitiesList() {
            $activities = $this->NSContactEventActivityCollection->activities;
            $responsibles=[];
            ...
            foreach($activities as $key => $activity){
                ...
                if(isset($activity['SETTINGS']['USERS'])){
                    foreach($activity['SETTINGS']['USERS'] as $collaboratorId){
                        ...
                        $activities[$key]['COWORKERS'][]=$this->NSContactEventActivityCollection->responsible[0];
                    }
                }
                $this->getResponsible($activity['RESPONSIBLE_ID']);
                unset($activities[$key]['SETTINGS']);
                $activities[$key]['responsible']=$this->NSContactEventActivityCollection->responsible[0]??null;
                $responsibles[]=$this->NSContactEventActivityCollection->responsible[0]??null;
            }
            

            $errorMessages=$this->errorMessages;
            $NSContactEventActivityCollection= $this->NSContactEventActivityCollection;
            include dirname(__FILE__) . '/template.php';
    }
    public function getCollection(){
        return $this->NSContactEventActivityCollection;
    }
}
